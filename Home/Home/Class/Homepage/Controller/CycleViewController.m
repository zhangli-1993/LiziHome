...
- (void)viewDidLoad {
    ...
    __weak CycleViewController *weakSelf = self;
...
#pragma mark---数据请求
//[NSString stringWithFormat:@"%@%@/posts?limit=20&gender=2&generation=1&offset=%ld", kKind, self.idStr, offset]
- (void)requestModel{
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/html",@"text/json", @"text/javascript", nil];
    [manager GET:[NSString stringWithFormat:@"%@%@/posts?limit=20&gender=2&generation=1&offset=%ld", kKind, self.idStr, offset] parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        //ZLLog(@"%@", responseObject);
        NSDictionary *dic = responseObject;
        NSDictionary *DataDic = dic[@"data"];
        for (NSDictionary *dict in DataDic[@"posts"]) {
